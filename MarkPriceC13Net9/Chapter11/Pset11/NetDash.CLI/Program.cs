        ...
        while(isRunning)
        {
            ...
            switch (choice)
            {
                ...
                case "server":
                    ...
                    if (db.Servers.Any(s => s.IpAddress == ipv4Address))
                    {
                    ...
                    Server server = ServerFactory.Create(ipv4Address, serverType, serverName, memorySize, dbEngine);
                ...
                case "analytics":
                    ...
                    var dbServersList = db.Servers
                        .OfType<DatabaseServer>()
                        .OrderByDescending(Server => Server.MemoryCapacity);

                    WriteLine($"\n*** List of Database Servers ***");
                    
                    foreach (var serv in dbServersList)
                    {
                        WriteLine($"{serv.Name, -10} {serv.MemoryCapacity, -10} {serv.DatabaseEngine, -10}");
                    }

                    var networkingList = db.Servers.Select(s => new { s.Name, s.IpAddress });
                    WriteLine("\n*** Server IP Addresses ***");
                    
                    foreach (var network in networkingList)
                    {

                        WriteLine($"{network.Name, -10} {network.IpAddress, -10}");
                    }

                    var serverStatus = db.Servers
                        .AsEnumerable()
                        .CountBy(s => s.CurrentStatus);

                    WriteLine("\n*** Server Status Count ***");

                    foreach (var status in serverStatus)
                    {
                        WriteLine($"{status.Key, -10} {status.Value}");
                    }
